...
def Navier_Stokes_LDC(Re, nx):
    ...
    #u_top = Expression('Re*8*((x[0]**4-2*x[0]**3+x[0]**2))', Re=Re, degree = 2)
    u_top = Expression(("Re*8.0*(pow(x[0],4.0)-2.0*pow(x[0],3.0)+pow(x[0],2.0))","0.0"), Re=Re, degree = 2)

    #print(u_top)
    noslip  = DirichletBC(W.sub(0), (0, 0), "x[0] < DOLFIN_EPS || x[0] > 1.0 - DOLFIN_EPS || x[1] < DOLFIN_EPS")
    lid  = DirichletBC(W.sub(0), u_top, "x[1] > 1.0 - DOLFIN_EPS")
